...
function usage() {
  return [
    "Usage:",
    "  create_note_draft.mjs [--root PATH] [--date YYYY-MM-DD] [--dry-run] <slug> <zh-title> <en-title>",
  ].join("\n");
}
...
function makeEnBody(title) {
  return [
    `# ${title}`,
    "",
    "Capture the idea first, then add context and the next step.",
    "",
    "## Note",
    "- ",
    "",
...
async function main() {
  ...
  const zhPath = path.join(repoRoot, "src/content/notes/zh", `${slug}.md`);
  const enPath = path.join(repoRoot, "src/content/notes/en", `${slug}.md`);
  ...
  console.log(`notes draft ready: ${path.relative(repoRoot, zhPath)} | ${path.relative(repoRoot, enPath)}`);
}
